...
public class AuthorDao implements IAuthorDao {
  ...
  public void addAuthor(String authorName, AuthorState authorState) throws DaoException {
     ...
     * connection.prepareStatement("INSERT INTO author (author, state) VALUES(?, ?)")) {
     * ps.setString(1, authorName); ps.setString(2, authorState.toString()); ps.executeUpdate(); }
     * catch (SQLException e) { logger.info("SQL Exception"); e.printStackTrace(); }
     */
  ...
  public void updateAuthorState(Author author, AuthorState authorState) throws DaoException {
    /*
     * try (Connection connection = ConnectorDb.getConnection(); PreparedStatement ps =
     * connection.prepareStatement("UPDATE author SET state = ? WHERE author = ?")) {
     * ps.setString(1, authorState.toString()); ps.setString(2, authorName); ps.executeUpdate(); }
     * catch (SQLException e) { logger.info("SQL Exception"); e.printStackTrace(); }
     */
  ...
  /*
   * @Override public void getAuthorIdByName(String authorName) throws DaoException { try
   * (Connection connection = ConnectorDb.getConnection(); PreparedStatement ps =
   * connection.prepareStatement("SELECT id FROM author WHERE author = ?")){ ps.setString(1,
   * authorName); ps.executeQuery(); } catch (SQLException e) { logger.info("SQL Exception");
   * e.printStackTrace(); } }
   */
  ...
  public Author getAuthorById(int authorId) throws DaoException {
    /*
     * ResultSet rs = null; Author author = new Author(); try (Connection connection =
     * ConnectorDb.getConnection(); PreparedStatement ps =
     * connection.prepareStatement("SELECT * FROM author WHERE a_id = ?")){ ps.setInt(1, authorId);
     * rs = ps.executeQuery(); while (rs.next()){ int i = rs.getInt(3); String name =
     * rs.getString(1); String authorState = rs.getString(2); author.setId(i); author.setName(name);
     * author.setAuthorState(AuthorState.valueOf(authorState)); } } catch (SQLException e) {
     * logger.info("SQL Exception select author by Id"); throw new DaoException(e); }
     */
    Author author = entityManager.find(Author.class, authorId);
    return author;
  }

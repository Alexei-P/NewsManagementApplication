...
public class UserDao implements IUserDao {
  ...
  public User signIn(String login, String password) throws DaoException {
    ...
    try (Connection connection = ConnectorDb.getConnection();
        PreparedStatement ps = connection.prepareStatement(
            "SELECT u_name FROM news_user where u_login = ? and u_password = ?")) {
    ...
    if (isUserExist == true) {
      try (Connection connection = ConnectorDb.getConnection();
          PreparedStatement ps =
              connection.prepareStatement("DELETE FROM news_user WHERE u_login = ?")) {
        ps.setString(1, login);
        ps.executeUpdate();
      } catch (SQLException e) {
        logger.info("Connection/Statement was not created");
  ...
  public boolean isUserExist(String login) throws DaoException {
    ...
    try (Connection connection = ConnectorDb.getConnection();
        PreparedStatement ps =
            connection.prepareStatement("SELECT u_name FROM news_user where u_login = ?")) {
      ps.setString(1, login);
      rs = ps.executeQuery();
      if (rs.next()) {
        isExist = true;
